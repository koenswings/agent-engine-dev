...
-   **Purpose:** Resets the state of an Engine. It can clear the data store, reset the engine's identity, or perform a hard reset by re-cloning the codebase from GitHub.
-   **Usage:** `./script/reset-engine [options] [engineName]`
-   **Options:**
    -   `--data`: Clears the Automerge data store.
    -   `--identity`: Resets the Automerge identity (UUIDs).
    -   `--meta`: Resets the `META.yaml` file (Engine Identity).
    -   `--code`: Performs a full code reset (re-clones the repo, preserves config).
